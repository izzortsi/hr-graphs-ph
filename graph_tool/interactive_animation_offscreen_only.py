...
##
doa_to_size = prop_to_size(g.vp.doa, mi=2, ma=10)
##
doa_to_size
##
win = Gtk.OffscreenWindow()
win.set_default_size(800, 800)
win.graph = GraphWidget(g, pos, vertex_fill_color=doa_to_size, vertex_size=doa_to_size)
win.add(win.graph)
...
def update_state():
    ...
    for i in range(100):
        ...
        if (
            norm(pos[s1].a - pos[t2].a) <= norm(pos[s1].a - pos[t1].a)
            and s1 != t2
            and t1.out_degree() > 1  # no self-loops
            and t2 not in s1.out_neighbors()  # no isolated vertices
        ):  # no parallel edges
